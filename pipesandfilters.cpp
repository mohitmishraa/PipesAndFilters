...
{
	static std::deque<Packet> d;
	return d;
}*/
Packet funcForFilter(Packet p,int filter_oper)
{
	int i;
	i = p.getSeqNo();
	i=i+filter_oper;
...
int DataSink::size()
{
	...
	//packet1.setSeqNo(2);
	//packet2.setSeqNo(3);
	//received.push_front(packet1);
	//received.push_front(packet2);
	//received.pop_back();
	//Packet pkt;
	//pkt = input -> read();
